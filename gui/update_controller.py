...
from gui.constants import FG_SECONDARY, UI_FONT
...
class UpdateController:
    ...
    def _on_silent_result(self, result: tuple[str, str, str]) -> None:
    ...
    def _handle_error(self, code: str | None, message: str, detail: str) -> None:
    ...
    def _handle_result(self, result: tuple[str, str, str] | None) -> None:
        log.info("_handle_result: %s", result)
        if result is None:
            self.app.statusBar().showMessage("Nessun aggiornamento")
            QMessageBox.information(self.app, "Aggiornamenti", f"MyNotes v{VERSION} e' aggiornato!")
            return
        tag, url, notes = result
        self._show_update_available(tag, url, notes)

    def _show_update_available(self, tag: str, url: str, notes: str) -> None:
    ...
    def _do_update(self, download_url: str) -> None:
        ...
        signals.finished.connect(lambda success: self._on_update_finished(success, last_error[0], progress_dlg, app))
    ...
    def _on_update_finished(self, success: bool, error_msg: str | None, progress_dlg: QDialog, app: MyNotesApp) -> None:
        progress_dlg.close()
        if success:
            import sys
            ...
            if is_windows:
                ...
                if (
                    QMessageBox.question(app, "Completato", "Aggiornamento applicato!\nRiavviare ora?")
